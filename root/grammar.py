...
def generate_transformed_segment(segment, tag, password_key):
    ...
    if segment.dictset_id <= 60:
        ...
        transformed_segment = database.get_dictionary_word(segment.dictset_id, int(dictionary_position))
    else:
        ...
        if database.check_wordset_pos(parent_pos) and parent_pos is not None:
            ...
            transformed_segment = database.get_wordlist_word(parent_pos, wordlist_position)
        else:
            # either a number, garbled characters or special symbols
            # check tag
            if "number" in tag:
                count = int(tag.split("number")[1])
                transformed_segment = generate_n_digit_random(count)
            elif "char" in tag:
                count = int(tag.split("char")[1])
                transformed_segment = generate_n_char_random(count)
            elif "special" in tag:
                count = int(tag.split("special")[1])
                transformed_segment = generate_n_symbol_random(count)
            else:
                print segment.word
                print tag
...
def main(db, transformed_cred_id, pwset_id, dryrun, verbose, basepath, tag_type, **kwargs):
    ...
    password_key = database.get_password_key(pwset_id)
